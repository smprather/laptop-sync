...
Push and pull run on independent schedules (`push_interval` / `pull_interval`, falling back to `interval`). Config is loaded from YAML (`laptop_sync.yaml` default), with explicit `push_enable` / `pull_enable` booleans and CLI flags as overrides. Exclude patterns are YAML-only. Local pull destinations can be symlinks to directories.
